...
async function main() {
    try {
        ...
        // var date = new Date();
        // console.log("Begin time >> ",date.getHours(),':',date.getMinutes(),':',date.getSeconds(),':',date.getMilliseconds())
        // var xmldata = fs.readFileSync('/data/RBI/Unprocessed_files/EP/EP011000000000000020161227020502.xml')
        ...
        // console.log(bcinfo)
        var blockitem = await channel.queryBlockByHash(bcinfo.previousBlockHash);
        // var prevblockitem = await channel.queryBlockByHash(bcinfo.previousBlockHash)
        // var blockcount= 4494;
        // // var totalids=0;
        // while(blockcount>2){
            // var blocknumber = await channel.queryBlock(46594);
        //     // totalids+= blocknumber.data.data[0].payload.data.actions[0].payload.action.proposal_response_payload.extension.results.ns_rwset[0].rwset.writes.length;
        ...
        //     if(blocknumber.data.data[0].payload.data.actions[0].payload.action.proposal_response_payload.extension.results.ns_rwset[0].rwset.writes.length == 0)
        //     {
        //         console.log(blocknumber)
        //         process.exit(1)
        //     }
        //     blockcount--;
        //     // console.log(blocknumber.data.data[0].payload.data.actions[0].payload.action.proposal_response_payload.extension.results.ns_rwset[0].rwset.writes)
        // }
        // console.log('toalids= '+totalids)
        // console.log(bcinfo)
        ...
        // console.log('\n------\n')
        var date1 = new Date();
        console.log("Begin time >> ",date1.getHours(),':',date1.getMinutes(),':',date1.getSeconds(),':',date1.getMilliseconds())
